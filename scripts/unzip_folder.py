...
parser.add_argument(
    ...
    dest="zipdir",
    type=Path,
    help='The folder containing the zip files, defaults to "output.tmp"',
    default="output.tmp",
)
parser.add_argument(
    "-outdir",
    dest="outdir",
    type=Path,
    help="The folder to unzip the files into. When not specified, use the same folder as specified in -zipdir",
)
parser.add_argument(
    "-test",
    action="store_true",
    dest="test",
    help="previews the zip operation without performing it",
)
parser.add_argument(
    "-remove-zip",
    action="store_true",
    dest="removeZip",
    help="remove the zip file, false by default",
)
