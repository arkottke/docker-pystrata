...
def test_get_files(bucket: str):
    ...
    s3lib.get_files(
        bucket_name=bucket, prefix="input/Dam001/Period01.7z", local_path="test1.tmp"
    )

    print("1 file, get by full name, to local full name")
    s3lib.get_files(
        bucket_name=bucket,
        prefix="input/Dam001/Period01.7z",
        local_path="test1.tmp/Period01.7z",
    )
    ...
    s3lib.get_files(
        bucket_name=bucket,
        prefix="input/Dam001/Period01.7z",
        local_path="test1.tmp/Period01_file.7z",
    )
    ...
    s3lib.get_files(
        bucket_name=bucket,
        prefix="input/Dam001",
        local_path="test2.tmp/Period01.7z",
        filter="Period01",
    )
    ...
    print("32 files, get by partial prefix, local path = directory name")
    s3lib.get_files(
        bucket_name=bucket, prefix="input/Dam00", local_path="test1.tmp", threads=40
    )
...
def test_put_files(bucket: str):
    ...
    s3lib.put_files(
        bucket_name=bucket, prefix="test1_out", local_folder="test1.tmp", threads=40
    )
