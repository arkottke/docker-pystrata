...
parser.add_argument(
    "-mode",
    dest="mode",
    help="The mode to run in, if not otherwise specified",
    default="",
)
parser.add_argument(
    "-bucket",
    dest="bucket",
    help="The name of an S3 bucket to use during the process, default is the $BUCKET_NAME environment variable",
...
parser.add_argument(
    ...
    dest="apply",
    action="store_true",
    help="Apply the current configuration, otherwise previews the outcome",
)
parser.add_argument(
    "-jobdef",
    dest="jobDef",
    help="The name of the job definition to use when running the batch job",
    default=os.getenv("BATCH_JOBDEF"),
)
parser.add_argument(
    "-queue",
    dest="jobQueue",
    help="The mode to run in, if not otherwise specified",
    default=os.getenv("BATCH_QUEUE"),
)
parser.add_argument(
    "-prefix",
    dest="itemPrefix",
    help="a prefix to use when the workitems parameter provides a numeric value, a numeric range or comma-separated list (optional)",
    default="",
)
parser.add_argument(
    "-padding",
    dest="itemPadding",
    type=int,
    help="when the workitems parameter provides a numeric value, a numeric range or comma-separated list, the number of padded zeros (optional)",
    default=3,
)
parser.add_argument(
    "-name-value",
    ...
    dest="name_values",
    help="one or more custom name=value pairs to pass as variables to the server process",
)
...
group1.add_argument(
    "-workitems",
    dest="items",
    help='An individual, list or range of work items, e.g. "item1"',
)
group1.add_argument(
    "-workitemfile",
    dest="itemListFile",
    type=argparse.FileType("r"),
    help="A file containing the list of items to run, each line in the format needed by the application",
)
...
def main(args):
    ...
    for work_item in work_items:
        count = count + 1
        print(
            f"starting: region [{client.meta.region_name}] bucket [{args.bucket}] job [{str(count).zfill(4)}] with [{work_item}] in mode [{args.mode}]"
        )
        ...
        if args.apply:
            ...
            print(
                f'job created. name [{response["jobName"]}], id [{response["jobId"]}]'
            )
        else:
            print(
                f"preview: submit_job({job_name}, {job_def}, {job_queue}, {env_vars})"
            )
