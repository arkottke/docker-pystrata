...
parser.add_argument(
    "-jobprefix",
    dest="job_prefix",
    help="a prefix to use when the jobnames parameter provided is numeric, a numeric range or comma-separated list (optional)",
    default="",
)
...
group1.add_argument(
    "-jobnames",
    dest="items",
    help='An individual, list or range of job names, e.g. "item1"',
)
group1.add_argument(
    "-jobnamefile",
    dest="item_list_file",
                ...
                if args.stop_all or job_item["jobName"] in work_items:
                    ...
                    if args.apply:
                        response = client.terminate_job(
                            jobId=job_item["jobId"], reason="user cancelled"
                        )
