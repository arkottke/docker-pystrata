...
def compress_inputs(
    source_folder: Path = "source.tmp/",
    zip_folder: Path = "zipped.tmp/",
    level: int = 1,
    filter: str = None,
):
...
def upload(
    ...
    print(
        f"loading input data to bucket: s3://{bucket_name}/{prefix} from {local_folder}"
    )
    s3lib.put_files(
